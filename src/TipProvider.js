...
export default class TipProvider extends Component {
  ...
  animatePulse = () => {
    const { showItemPulseAnimation = this.props.showItemPulseAnimation } =
      this.state;
  ...
  onTipLayout = (e) => {
    ...
    const tipProps = getTipPositionProps(
      this.state.itemCoordinates,
      height,
      width
    );
  ...
  getTipAnimation() {
    const { pivotPoint = { x: 0, y: 0 } } = this.state;
        ...
        >
          {renderTip ? (
            ...
            <>
              ...
              {body && <Text style={_bodyStyle}>{body}</Text>}
            </>
          )}
          ...
          {tourProps && (
            <View style={styles.actions}>
              {!!tourProps.prevId && (
                <TouchableOpacity
                  activeOpacity={0.5}
                  onPress={() => TipManager.changeTipTour(tourProps, "prev")}
                  style={_prevNextButtonStyle}
                >
                  <Text style={_prevNextTextStyle}>{prevButtonLabel}</Text>
                </TouchableOpacity>
              )}
              ...
              {!!tourProps.nextId && (
                <TouchableOpacity
                  activeOpacity={0.5}
                  onPress={() => TipManager.changeTipTour(tourProps, "next")}
                  style={_prevNextButtonStyle}
                >
                  <Text style={_prevNextTextStyle}>{nextButtonLabel}</Text>
                </TouchableOpacity>
              )}
